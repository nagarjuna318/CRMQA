...
vlocity.cardframework.registerModule.factory('LWC', [function() {
    return {
        show_iframe : false,
        show_competitor_iframe : false,
        iframe_adjust : false
    }
}]);

vlocity.cardframework.registerModule.controller('custom_iframe_ctrl', ['$scope', '$rootScope', 'LWC', '$sce', function($scope, $rootScope, LWC, $sce) {

    $scope.lwc = LWC
    
    $scope.getVfPageURL = function(bpTreeResp){
        return $sce.trustAsResourceUrl(bpTreeResp.sfdcIFrameOrigin.replace("vlocity-ins", "c") + "/apex/LWCLightningOutToOSPage")
    }

}]);

vlocity.cardframework.registerModule.controller('custom_iframe_cotrl', ['$scope', '$rootScope', 'LWC', '$sce', function($scope, $rootScope, LWC, $sce) {

    $scope.lwc = LWC
    
    $scope.getVfPageURL = function(bpTreeResp){
        return $sce.trustAsResourceUrl(bpTreeResp.sfdcIFrameOrigin.replace("vlocity-ins", "c") + "/apex/LWCLightningOutToBenefitPage")
    }

}]);

vlocity.cardframework.registerModule.controller('insOsSmallGroupPlanSelectionCtrl', ['$scope', '$rootScope', '$timeout', '$q', '$sldsModal', 'DfilterService', 'SearchCriteria', 'LWC', function($scope, $rootScope, $timeout, $q, $sldsModal, DfilterService, SearchCriteria, LWC) {
    'use strict';
    ...
    $scope.sendMessage = function(bpTree) {
        // Make sure you are sending a string, and to stringify JSON
        // console.log('bpTree.response.selectedPlanArray'+bpTree.response.selectedPlanArray);
        // the below function is breaking and causing this whole function to error. 
        $scope.lwc.show_iframe = true
        var iframeEl = document.getElementById('lwcIframe');
        iframeEl.contentWindow.postMessage(bpTree.response.selectedPlanArray, '*');
        
    };

    // Listen to message from child window
    bindEvent(window, 'message', function (e) {
        // console.log('OmniScript received data - Vlocity Template => ', e.data, $scope.bpTree);
        if(!(e.data instanceof Object)) {
            ...
            if(newValue.closeWindow){
                ...
                $scope.lwc.iframe_adjust = false
                $scope.lwc.show_competitor_iframe = false
                $scope.$apply()
    ...
    $scope.insSelectionInit = function(baseCtrl, control, initializeCart) {
        ...
        control.insSgpsCustomEventName = 'vloc-os-ins-small-group-selection-' + control.name + Math.round((new Date()).getTime() / 1000);
            ...
            .then(function(remoteResp) {
                ...
                control[control.name].totalPlans = remoteResp[control.name].ratedProducts.totalSize;
                    ...
                    if(control[control.name].totalPlans > 0){
                        ...
                        var plan_list_for_networks = [...remoteResp[control.name].ratedProducts.records]
                    ...
                    riders_distinct.forEach(function(rider, index){
                        ...
                        new_filter_cond.Operator = "="
                        new_filter_cond.Type =  "Field"
                        new_filter_cond.Value = rider.value
                        new_filter_cond.is_checked =  false 
        ...
        angular.forEach(products, function(product, productIterator) {
            if (product[baseCtrl.prototype.$scope.nsPrefix  + 'RecordTypeName__c'] !== 'RatingFactSpec' && product.RecordTypeName__c !== 'RatingFactSpec') {
                ...
                if (product.attributeCategories && product.attributeCategories.records) {
                    ...
                    // if(product.rider_list.length > 0){
                    //     formattedContent.attributeRows.push({
                    //         label: "Rider Plan Name",
                    //         productCodes: [],
                    ...
                    var rider_string = "--"
                    if(product.rider_list.length > 0){
                        rider_string = product.rider_list.join(", ")
                    }
                        ...
                        if (attributeCategory.productAttributes && attributeCategory.productAttributes.records) {
                            ...
                            angular.forEach(attributeCategory.productAttributes.records, function(productAttribute) {
                                ...
                                if(!productAttribute.hidden){
                                    ...
                                    if(formattedContent.attributeIndices.hasOwnProperty("OTHER_riderName") && prod_code.includes("rider")){
                                        if (!formattedContent.attributeRows[formattedContent.attributeIndices["OTHER_riderName"]][product.ProductCode]) {
                                            formattedContent.attributeRows[formattedContent.attributeIndices["OTHER_riderName"]].attributeValues[productIterator] = {
                                                PlanId: product.Id,
                                                ProductCode: product.ProductCode,
                                                attributeCode: "OTHER_riderName",
                                                userValues: rider_string,
                                    ...
                                    }else if (formattedContent.attributeIndices.hasOwnProperty(productAttribute.code)) {
                                    ...
                                    } else if(productAttribute.label === "Rider Plan Name"){
                                        ...
                                        formattedContent.attributeIndices["OTHER_riderName"] = attributeRowsLength - 1;
    ...
    function reformatContent(formattedContent){
        ...
        angular.forEach(formattedContent.attributeRows, function(product, productIterator){
            ...
            //     re_formatted_content.plan_overview.push(product)
            
            // } else {
            
            //     re_formatted_content.plan_details.push(product)
    ...
    $scope.openCompareModal = function(bpTree, plan, control) {
        if (plan) {
            $scope.modalRecords = [plan, plan.originalPlan.records[0]];
            $scope.isSelectable = false;
        } else {
            const opptyid = bpTree.response.QuoteDetails.OpptyId
            ...
            var reformatted_plans_for_compare_modal = reformatContent(formatContent(all_plans_to_compare))
            reformatted_plans_for_compare_modal["OpptyId"] = opptyid
            reformatted_plans_for_compare_modal["ProductType"] = "Medical"
            reformatted_plans_for_compare_modal["Situs_State"] = $scope.bpTree.response.Situs_State
            $scope.lwc.show_iframe = true
            $scope.lwc.iframe_adjust = true
            var iframeEl = document.getElementById('lwcIframe');
            iframeEl.contentWindow.postMessage(reformatted_plans_for_compare_modal, '*');
        }
    };
var static_json = 
    {
      ...
      "categorized_list": {
        "Plan Overview": [
          ...
          {
            "label": "In Network Deductible Family",
            "attributeValues": []
          },
          {
            "label": "Co Pay - Primary Care Provider (INN)",
            "attributeValues": []
          },
          {
            "label": "In Network Deductible Single",
            "attributeValues": []
          },
          {
            "label": "In Network Out of Pocket Max Single",
            "attributeValues": []
          }
        ],
        "In Network": [
          {
            "label": "In Network Out of Pocket Max Family",
            "attributeValues": []
          }
        ],
        "Out of Network": [
          {
            "label": "Out of Network Deductible Single",
            "attributeValues": []
          },
          {
          ...
          {
            "label": "Out of Network Deductible Family",
            "attributeValues": []
          },
          {
            "label": "OON Out of Pocket Max Family",
            "attributeValues": []
          },
          {
            "label": "OON Out of Pocket Max Single",
            "attributeValues": []
          }
        ],
        "Specialty Care Provider": [
          {
            "label": "Co Pay - Specialty Care Provider (INN)",
            "attributeValues": []
          },
          {
            "label": "Co Ins - Specialty Care Provider (OON)",
            "attributeValues": []
          }
        ],
        "Primary Care Provider": [
          {
            "label": "Co Ins - Primary Care Provider (OON)",
            "attributeValues": []
          }
        ],
        "Emergency Room": [
          {
            "label": "Co Ins - Emergency Room (INN)",
            "attributeValues": []
          },
          {
            "label": "Co Pay - Emergency Room (INN)",
            "attributeValues": []
          }
        ],
        "Urgent Care": [
          {
            "label": "Co Pay - Urgent Care (INN)",
            "attributeValues": []
          },
          {
            "label": "Co ins -Urgent Care (OON)",
            "attributeValues": []
          }
        ]
      },
      "OpptyId": "0063C000008VxlzQAC",
      "ProductType": "Medical",
      "Situs_State": "CA"
    };

    $scope.openCompetitorModal = function(bpTree, plan, control) {
        const opptyid = $scope.bpTree.response.QuoteDetails.OpptyId
        static_json["OpptyId"] = opptyid
        static_json["ProductType"] = "Medical"
        static_json["Situs_State"] = $scope.bpTree.response.Situs_State
        $scope.lwc.iframe_adjust = true
        $scope.lwc.show_competitor_iframe = true
        let iframeEl = document.getElementById('lwcIframe_competitor1');
        iframeEl.contentWindow.postMessage(static_json, '*');
        // console.log(JSON.stringify(static_json))
    };
    ...
    $scope.clearSelection = function(control){
        ...
        //add any compare selected plans to the compare map attribute, this is a doublecheck in case some have been selected via the lwc
      ...
      /*  var selectedPlanArray;
        ...
        if(bpTree.response.selectedPlanArray != undefined){

            selectedPlanArray =bpTree.response.selectedPlanArray;

            selectedPlanArray.forEach(function(element,index){
            if(selectedPlanArray[index].vlcCompSelected === false){
                selectedPlanArray.splice(index, 1);
            }
        });
         bpTree.response.selectedPlanArray = selectedPlanArray;
        }
        */
