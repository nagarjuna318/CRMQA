...
vlocity.cardframework.registerModule.controller('editableTableTableAppController', function($scope, $modal, $tooltip, editableTableService, $dateParser, $timeout, $rootScope, InsModalService) {
    ...
    */
        ...
        $scope.sendSaveToServer(
            function(result) {
                ...
                if (showConfirmation) {
                    $rootScope.notification.active = true;
                    $rootScope.notification.type = 'success';
                    $rootScope.notification.message = 'Save Successful';
                    $timeout(function() {
                        $rootScope.notification.active = false;
                    }, 2000);
                }
            ...
            function() {
                ...
                if (showConfirmation) {
                    $rootScope.notification.active = true;
                    $rootScope.notification.type = 'error';
                    $rootScope.notification.message = 'Save Failed. Please fix errors and try again.';
                    $timeout(function() {
                        $rootScope.notification.active = false;
                    }, 2000);
                }
    ...
    $scope.sendSaveToServer = function(successCallback, failureCallback) {
        ...
        editableTableService.invokeRemoteMethod($scope, 'CensusMemberServiceHandler', 'saveData', inputMap, 'Save Successfully.').then(function(result, event) {
            if (result) {
                $scope.addNew = false;
                ...
                if (result.result == 'error') {
                    failureCallback();
                } else if ($scope.hasPendingToSave()) {
    ...
    */
    $scope.saveMember = function(row) {
        ...
        $timeout(function() {
            editableTableService.invokeRemoteMethod($scope, 'CensusMemberServiceHandler', 'saveData', inputMap, 'Updated Member Successfully.');
        }, 500);
    }
    ...
    $scope.deleteAllData = function() {
        for (let i = 0; i < $rootScope.tableData.rows.length; i++) {
            $rootScope.tableData.rows[i].deleted = true;
        }
    ...
    $scope.exportData = function() {
        ...
        angular.forEach($rootScope.tableData.rows, function(row) {
            if (!row.deleted) {
                var rowConverted = {};
                angular.forEach($rootScope.tableData.headers, function(header) {
                    var rowData = row[header.name];

                    if (header.type == 'DATE' && rowData != null) {
                        rowData = $scope.parseDate(row[header.name]);
                    }

                    rowConverted[header.label] = rowData;
                });
        ...
        editableTableService.invokeRemoteMethod($scope, 'CensusMemberServiceHandler', 'exportData', {
            exportCSV: exportInfo
        }).then(function(result, event) {
            if (result.result == 'success') {
                $rootScope.notification.active = true;
                $rootScope.notification.type = 'success';
                $rootScope.notification.message = 'Saved as Attachment. Save data and reload page to view.';
                $timeout(function() {
                    $rootScope.notification.active = false;
                }, 2000);
            } else {
                $rootScope.notification.active = true;
                $rootScope.notification.type = 'error';
                $rootScope.notification.message = 'Save Failed.';
                $timeout(function() {
                    $rootScope.notification.active = false;
                }, 2000);
            }
