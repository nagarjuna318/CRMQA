...
vlocity.cardframework.registerModule.controller('insCoveragesCtrl', ['$scope', '$rootScope', '$timeout', '$q', '$filter', 'InsRulesEvaluationService', function($scope, $rootScope, $timeout, $q, $filter, InsRulesEvaluationService) {
    'use strict';
    ...
    function changeCoverageHelper(remoteResp, product, productsFromAddHashKeys, shouldFormatData, pricesUpdated, response, control, scp, currentTextValue, sortFunc) {
        ...
        if ($scope.changeCoverageChain.moreQueued.length) {
            ...
            $scope.validationMessages = [];
            // Regenerate sortedCoverages for each set
            ...
            $timeout(function() {
                for (let key in $scope.sortedCoverages) {
                    InsRulesEvaluationService.doProductLoop($scope.sortedCoverages[key], false, true, true);
                }
     ...
     */
    $scope.isDoneDisabled = function(coverage) {
        ...
        link: function(scope, element, attrs) {
            ...
            const onClick = function(event) {
                ...
                if (attrs.isClickDisabled === 'true') {
                 ...
                 if(attrs.coveragePopoverOpen === 'false' && !(event.target.className.indexOf('vloc-coverage-edit') > -1)){
                    return; //return if popover is not open and click is not on edit button 
                } 
                if(event.target.className.indexOf('vloc-ins-coverages') > -1){ //if target element is in a vloc-ins-coverages tag it belongs to the modal, so return
                    return;
                }
                if (initial && isInside) {
                    return;
                ...
                if ((!isInside && className.indexOf(attrs.restrictElement) < 0) || (isInside && className.indexOf(attrs.restrictElement) < 0 && !attrs.hasOwnProperty('ngClick'))) {
                    ...
                    scope.$apply(attrs.insOsDropdownHandlerInit);
                    $document.off('click', onClick);
                    //if popover is not open - i.e. popover flag switched to false, save -- make sure target element is not edit button, should never invoke save on edit call
                    if(attrs.coveragePopoverOpen === 'false' && !(event.target.className.indexOf('vloc-coverage-edit') > -1)){ 
                        scope.$apply(attrs.changeCoverageFn);
                    }
                    focused = false;
                }
            };
